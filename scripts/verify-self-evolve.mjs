...
writeFileSync(join(home, "settings.json"), JSON.stringify({ selfEvolve: true, selfSourceRoot: source, agentAccess: "workspace", workspaces: { home: workspace } }));
const port = Number(process.env.HARNESS_SELF_QA_PORT || 31879), base = `http://127.0.0.1:${port}`;
...
try {
  ...
  console.log(JSON.stringify({ ok: true, provider: "deepseek-harness", access: "workspace", elapsedMs: Date.now() - started, toolCalls: events.filter(e => e.t === "tool").length, changedFiles: change.files.map(f => f.path), beforeCommit: change.beforeCommit, afterCommit: change.afterCommit, rollbackCommit: restored.checkpoint.afterCommit, remote: "none; disposable fixture" }, null, 2));
} finally {
  child.kill("SIGTERM");
