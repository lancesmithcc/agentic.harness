...
const DEFAULT_SETTINGS: HarnessSettings = { astraAvailable: false, theme: "gold", workspaces: {} };
...
function routedModels(all: OrchestratorContext["models"], s: HarnessSettings, orchestratorJob: boolean) {
  if (s.astraAvailable && orchestratorJob) return all;
  return all.filter((m) => !ORCHESTRATOR_ONLY_MODELS.has(m.id));
}
    ...
    if (url.pathname === "/api/status") {
      ...
      return json({
        ...
        providers: [...ctx.health.entries()].map(([id, h]) => ({
          ...
          ok: h.ok,
          detail: h.detail,
          models: ctx.models.filter((m) => m.provider === id).map((m) => ({
            id: m.id,
